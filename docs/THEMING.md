...
In mobile mode `ChatContainer` writes `--g-aikit-prompt-input-body-textarea-max-height` onto its root, measured as the root height minus the header and minus the part of the footer that is not the textarea. It is written whatever the on-screen keyboard is doing: a field that grew while the keyboard was away would otherwise come back taller than the space left for it.
